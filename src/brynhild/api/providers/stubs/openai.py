"""
OpenAI direct API provider stub.

This provider is not yet implemented. When implemented, it will provide
direct access to OpenAI's API without going through OpenRouter.
"""

from __future__ import annotations

import typing as _typing

import brynhild.api.base as base
import brynhild.api.types as types
import brynhild.constants as _constants


class OpenAIProvider(base.LLMProvider):
    """
    OpenAI direct API provider (NOT IMPLEMENTED).

    This provider will support direct OpenAI API access.
    Currently, use OpenRouter to access OpenAI models.
    """

    PROVIDER_TYPE = "openai"

    def __init__(self, **kwargs: _typing.Any) -> None:
        raise NotImplementedError(
            "OpenAI direct provider is not yet implemented.\n\n"
            "Options:\n"
            "  1. Use OpenRouter to access OpenAI models:\n"
            "     providers.default: openrouter\n"
            "     models.default: openai/gpt-oss-120b\n\n"
            "  2. Contribute an implementation at:\n"
            "     https://github.com/your-repo/brynhild"
        )

    @property
    def name(self) -> str:
        return "openai"

    @property
    def model(self) -> str:
        return ""

    def supports_tools(self) -> bool:
        return True

    async def complete(
        self,
        messages: list[dict[str, _typing.Any]],
        *,
        system: str | None = None,
        tools: list[types.Tool] | None = None,
        max_tokens: int = _constants.DEFAULT_MAX_TOKENS,
        use_profile: bool = True,
    ) -> types.CompletionResponse:
        raise NotImplementedError("OpenAI provider not implemented")

    async def stream(  # noqa: ARG002
        self,
        messages: list[dict[str, _typing.Any]],
        *,
        system: str | None = None,
        tools: list[types.Tool] | None = None,
        max_tokens: int = _constants.DEFAULT_MAX_TOKENS,
        use_profile: bool = True,
    ) -> _typing.AsyncIterator[types.StreamEvent]:
        raise NotImplementedError("OpenAI provider not implemented")
        yield  # Make this a generator (unreachable)


