    ...
    def update_token_counts(self, input_tokens: int, output_tokens: int) -> None:
        ...
        Args:
            input_tokens: Current context window size (absolute, not cumulative).
                This is the total prompt tokens for the most recent API call,
                which represents how much of the context window is currently used.
            output_tokens: Cumulative tokens generated by the model across the session.
        """
    ...
    def show_prompt_source(self, sources: list[str], content: str) -> None:
        ...
        def format_source(s: str) -> str:
            if s.startswith("file:"):
                return s[5:]  # Remove "file:" prefix, show path
            elif s == "stdin":
                return "stdin"
            elif s == "args":
                return "args"
        ...
        if len(sources) == 1:
            source_info = format_source(sources[0])
        else:
